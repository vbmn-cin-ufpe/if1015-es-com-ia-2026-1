...
import { Card, ProgressBar, ThinkingDots, EmptyState, btnPrimary, btnSecondary, inputCls } from "../ui"
...
export function TourTab({ repositoryId, status }: Props) {
  ...
  if (status !== "completed") {
    return (
      <Card>
        <EmptyState icon="🗺️" title="Indexe um repositório primeiro" description="O tour guiado estará disponível após a indexação ser concluída." />
      </Card>
    )
  }
  ...
  if (!tour) {
    return (
      <div className="space-y-4">
        <Card title="🗺️ Configurar Tour Guiado">
          ...
          <div className="mb-5">
            <div className="flex justify-between mb-1">
              <label className="text-sm font-medium text-gray-700">Número de módulos no tour</label>
          ...
          {/* Pesos */}
          <p className="text-sm font-semibold text-gray-700 mb-3">Critérios de seleção dos módulos</p>
          <div className="space-y-4 mb-4">
            {[
              { label: "Complexidade do código", desc: "Prioriza módulos com maior complexidade ciclomática", val: complexityW, set: setComplexityW },
              { label: "Frequência de mudanças", desc: "Prioriza módulos mais frequentemente modificados", val: churnW, set: setChurnW },
              { label: "Acoplamento entre módulos", desc: "Prioriza módulos com mais dependências", val: couplingW, set: setCouplingW },
              ...
              <div key={label}>
                <div className="flex justify-between mb-1">
                  ...
                  <span className="text-sm font-bold text-indigo-600 ml-4 shrink-0">{(val * 100).toFixed(0)}%</span>
                </div>
          ...
          <div className="flex items-center justify-between pt-3 border-t border-gray-100">
            <span className={`text-sm font-medium ${weightsOk ? "text-green-600" : "text-amber-600"}`}>
              {weightsOk ? "✓ Pesos válidos (soma = 100%)" : `⚠ Soma atual: ${(weightSum * 100).toFixed(0)}% (deve ser 100%)`}
            </span>
            <button onClick={onGenerate} disabled={loading} className={btnPrimary}>
              🗺️ Gerar Tour Guiado
            </button>
          </div>
        ...
        {toursLoaded && savedTours.length === 0 && (
          <p className="text-sm text-gray-400 text-center">Nenhum tour salvo ainda.</p>
        )}
      </div>
    )
  }
    ...
    <div className="space-y-4">
      ...
      <Card>
        <div className="flex items-start justify-between mb-4">
          ...
          <button onClick={() => setTour(null)} className="text-gray-300 hover:text-gray-500 text-2xl ml-4 leading-none">×</button>
        </div>

        {/* Progress */}
        <div className="mb-1 flex justify-between text-xs text-gray-500">
          <span>Módulo {step + 1} de {tour.step_count}</span>
          ...
          {tour.steps.map((_, i) => (
            <button
              ...
              className={`h-1.5 rounded-full transition-all ${
                i < step ? "bg-indigo-300" : i === step ? "bg-indigo-600 w-6" : "bg-gray-200"
              } w-${i === step ? "6" : "3"}`}
      ...
      {currentStep && (
        <Card className="border-l-4 border-indigo-500">
          ...
          <p className="text-xs font-mono text-gray-400 mb-4">{currentStep.module_name}</p>
          ...
          <div className="mb-4 bg-amber-50 border border-amber-100 rounded-lg p-4">
            ...
            <p className="text-sm text-amber-900">{currentStep.rationale}</p>
          </div>
            ...
            <div className="mt-3 grid grid-cols-3 gap-3 text-center">
              <div className="bg-red-50 rounded-lg border border-red-100 p-3">
                <p className="text-xl font-bold text-red-700">
                  {(currentStep.metrics.complexity["avg_complexity"] as number | undefined)?.toFixed(1) ?? "—"}
                </p>
                <p className="text-xs text-red-500 mt-0.5">Complexidade média</p>
              </div>
              <div className="bg-blue-50 rounded-lg border border-blue-100 p-3">
                <p className="text-xl font-bold text-blue-700">
                  {(currentStep.metrics.churn["total_commits"] as number | undefined) ?? 0}
                </p>
                <p className="text-xs text-blue-500 mt-0.5">Commits totais</p>
              </div>
              <div className="bg-purple-50 rounded-lg border border-purple-100 p-3">
                <p className="text-xl font-bold text-purple-700">
                  {(currentStep.metrics.coupling["unique_dependencies"] as number | undefined) ?? 0}
                </p>
                <p className="text-xs text-purple-500 mt-0.5">Dependências únicas</p>
              </div>
            </div>
          </details>
        </Card>
      ...
      <div className="flex items-center gap-3">
        <button onClick={() => setStep(s => s - 1)} disabled={step === 0} className={btnSecondary}>
          ← Anterior
        </button>
        <span className="text-sm text-gray-400 flex-1 text-center">{step + 1} / {tour.step_count}</span>
        {step < tour.step_count - 1 ? (
          <button onClick={() => setStep(s => s + 1)} className={btnPrimary}>
            Próximo →
          </button>
        ) : (
          <button onClick={() => setTour(null)} className={`${btnPrimary} bg-green-600 hover:bg-green-700`}>
            ✓ Concluir tour
          </button>
        )}
      </div>
