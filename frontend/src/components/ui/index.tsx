...
export function Card({
  ...
  return (
    <div className={`bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6 ${className}`}>
      {title && <h2 className="text-lg font-semibold text-gray-800 dark:text-gray-100 mb-4">{title}</h2>}
      {children}
    </div>
  )
}
...
const STATUS_COLORS: Record<string, string> = {
  ...
  indexing:   "bg-yellow-100 text-yellow-800 dark:bg-yellow-900/40 dark:text-yellow-400",
  ...
  excellent:  "bg-emerald-100 text-emerald-800 dark:bg-emerald-900/40 dark:text-emerald-400",
}
  ...
  return (
    <div className="flex items-center gap-2 text-sm text-indigo-600 dark:text-indigo-400">
      ...
      <span className="flex gap-0.5">
        <span className="animate-bounce w-1 h-1 bg-indigo-400 dark:bg-indigo-500 rounded-full" style={{ animationDelay: "0ms" }} />
        <span className="animate-bounce w-1 h-1 bg-indigo-400 dark:bg-indigo-500 rounded-full" style={{ animationDelay: "150ms" }} />
        <span className="animate-bounce w-1 h-1 bg-indigo-400 dark:bg-indigo-500 rounded-full" style={{ animationDelay: "300ms" }} />
      </span>
    </div>
  )
}
...
export function ErrorBanner({ message, onClose }: { message: string; onClose: () => void }) {
  return (
    <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg px-4 py-3 flex items-center justify-between">
      ...
      <button onClick={onClose} className="text-red-400 hover:text-red-600 dark:hover:text-red-300 ml-4 text-lg leading-none">
        ×
      </button>
    </div>
  )
