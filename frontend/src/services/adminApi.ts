...
export async function listUsers(options?: {
  ...
  return httpGet<UserListResponse>(`/api/admin/users${qs}`, { headers: authHeader() })
}

export async function getAdminUser(userId: string): Promise<UserSummary> {
  return httpGet<UserSummary>(`/api/admin/users/${userId}`, { headers: authHeader() })
}
...
export async function resetUserPassword(userId: string): Promise<{ message: string }> {
  const { env } = await import("../infrastructure/env")
  const resp = await fetch(`${env.apiBaseUrl}/api/admin/users/${userId}/reset-password`, {
    method: "POST",
    headers: authHeader(),
  })
  if (!resp.ok) throw new Error(await resp.text())
...
export async function getReposHealth(): Promise<ReposHealthResponse> {
  return httpGet<ReposHealthResponse>("/api/admin/repos/health", { headers: authHeader() })
}
...
export async function getUsageDashboard(days = 30): Promise<UsageSummary> {
  return httpGet<UsageSummary>(`/api/admin/usage?days=${days}`, { headers: authHeader() })
}
...
export async function getLlmFeedback(days = 30): Promise<LlmFeedbackResponse> {
  return httpGet<LlmFeedbackResponse>(`/api/admin/llm-feedback?days=${days}`, { headers: authHeader() })
}
...
export async function getLlmCosts(days = 30): Promise<LlmCostSummary> {
  return httpGet<LlmCostSummary>(`/api/admin/llm-costs?days=${days}`, { headers: authHeader() })
}
...
export async function getIngestionQueue(): Promise<IngestionQueueResponse> {
  return httpGet<IngestionQueueResponse>("/api/admin/ingestion-queue", { headers: authHeader() })
}
...
export async function updatePlanLimit(
  plan: string,
  body: { max_repos: number; max_questions: number; can_delete_repo: boolean }
): Promise<PlanLimit> {
  return httpPatch<PlanLimit>(`/api/admin/plans/${plan}`, body, { headers: authHeader() })
}
...
export async function getAuditLog(params: {
  ...
  const qs = Object.entries(params)
    ...
    .join("&")
  return httpGet<AuditLogResponse>(`/api/admin/audit-log${qs ? "?" + qs : ""}`, { headers: authHeader() })
}
