...
import { signin, signup, signout, listSessions, createSession, type SessionInfo } from "./services/authApi"
...
import { Badge, inputCls, btnPrimary, btnSecondary } from "./components/ui"
    ...
    <div className="flex flex-col h-screen bg-gray-50 dark:bg-gray-900 font-sans overflow-hidden">
      ...
      <header className="flex-shrink-0 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 sticky top-0 z-20 shadow-sm">
        <div className="px-4 h-14 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            {authUser ? (
              <>
                <span className="text-sm text-gray-600 dark:text-gray-300 hidden sm:inline">{authUser.email}</span>
                {repositoryId && (
                  <button onClick={onCreateSession} className={btnSecondary + " text-xs"}>+ Sessão</button>
                )}
                <button onClick={onSignout} className={btnSecondary + " text-xs"}>Sair</button>
              </>
            ) : (
              <button onClick={() => setAuthOpen(o => !o)} className={btnSecondary + " text-xs"}>
                🔐 Entrar
              </button>
            )}
          </div>
        ...
        {authOpen && !authUser && (
          <div className="absolute right-4 top-14 w-80 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl shadow-xl p-5 z-30">
            <h3 className="text-sm font-bold text-gray-800 dark:text-gray-100 mb-3">Entrar / Criar conta</h3>
            <form className="space-y-3">
              ...
              <input value={authPassword} onChange={e => setAuthPassword(e.target.value)}
                placeholder="Senha" type="password" className={inputCls} />
              {authError && <p className="text-xs text-red-500">{authError}</p>}
              <div className="flex gap-2">
                <button type="submit" onClick={onSignin} className={btnPrimary + " flex-1 justify-center"}>Entrar</button>
                <button type="button" onClick={onSignup} className={btnSecondary + " flex-1 justify-center"}>Cadastrar</button>
              </div>
            </form>
          </div>
        )}
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <aside className={
          ...
          <nav className="flex-1 py-3 space-y-0.5 px-2 overflow-y-auto overflow-x-hidden">
            {TABS.map(tab => (
              ...
              >
                <span className="text-lg flex-shrink-0 w-6 text-center">{tab.icon}</span>
        ...
        <main className="flex-1 overflow-auto px-6 py-6">
          {activeTab === "repo" && (
            <RepoTab repositoryId={repositoryId} status={status}
              onIndexed={(id, s) => { setRepositoryId(id); setStatus(s) }} />
          )}
          {activeTab === "chat"    && <ChatTab    repositoryId={repositoryId} status={status} />}
          {activeTab === "tour"    && <TourTab    repositoryId={repositoryId} status={status} />}
          {activeTab === "graph"   && <GraphTab   repositoryId={repositoryId} status={status} />}
          {activeTab === "history" && <HistoryTab repositoryId={repositoryId} status={status} />}
          {activeTab === "metrics" && <MetricsTab repositoryId={repositoryId} status={status} />}
